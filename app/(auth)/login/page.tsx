...
function SignInForm() {
  ...
  // Check for error in URL (from NextAuth redirects)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!isLogin) {
        ...
        // (we fall through to the signIn call below)
      }
      ...
      const result = await signIn('credentials', {
        email,
        password,
        redirect: false, // Handle manually for better UX
      })

      if (result?.error) {
        // Common NextAuth error for wrong credentials
